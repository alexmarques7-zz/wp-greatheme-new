<div class="m-depoimentos d-bg-brand-color-two py-5">
    <div class="container-fluid py-4">
        <div class="container">
            <div class="row">
                <div class="col-6">
                    <h2 class="d-text-color-light d-font-bold d-text-xxxl mb-4 w-75">Veja o que os produtores estão falando</h2>
                    <p class="d-text-color-light d-font-regular d-text-lg mb-4">Quer mais informações sobre a plataforma?</p>
                    <a class="e-btn --white" href="#">Fale com o Consultor</a>
                </div>
                <div class="col-6">
                    <div class="e-card-color --white">
                        <img class="e-circle" src="" alt="">
                        <div>
                            <p>"Através da Aulapp consegui levar meus cursos e palestras para o meio digital. É uma plataforma incrível, intuitiva e com recursos que permitem personalizar a estrutura de cada curso." Débora Guerreiro Owner G5 Brasil</p>
                            <p>
                                Débora Guerreiro
                                <span>Owner G5 Brasil</span>
                            </p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div> 